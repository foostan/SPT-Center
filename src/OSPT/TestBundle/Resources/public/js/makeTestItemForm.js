...
var questionCH = $('div#test_item_test_iteQuestions');
...
$(document).ready(function() {
    ...
    questionCH.find('div.question').each(function() {
		...
		var choiceCH = $(this).find('div:regex(id,^test_item_test_iteQuestions_[0-9]+_test_iteQueChoices$)');
		...
		function addChoiceForm() {
			...
			var newForm = choiceCH.attr('data-prototype').replace(/\$\$test_iteQueChoice\$\$/g, choiceCH.children().length-1).replace(/\$\$test itequechoice\$\$/g, '');
...
function addQuestionForm() {
    ...
    // Replace '$$name$$' in the prototype's HTML to
    // instead be a number based on the current collection's length.
   	var newForm = prototype.replace(/\$\$test_iteQuestion\$\$/g, questionCH.children().length-1).replace(/Test itequechoices/g, '<h3>Choices</h3>');
	...
	// choice
	var choiceCH = $newFormDiv.find('div:regex(id,^test_item_test_iteQuestions_[0-9]+_test_iteQueChoices$)');
	var $addChoiceLink = $('<a href="#" class="add_choice_link btn btn-mini btn-primary">Add choice</a>');
	var $newChoiceLinkDiv = $('<div class="clear"></div>').append($addChoiceLink);
